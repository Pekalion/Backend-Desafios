window.addEventListener('load', async () => {
  ...
  if (response.status !== 200) {
    alert('¡Necesitas iniciar sesión para ver esta información!');
...
async function navSetup(opcionesPaginacion) {
  const { page, totalPages, hasNextPage, hasPrevPage, nextPage, prevPage } = opcionesPaginacion;
...
function prepareFront() {
  fetch('http://localhost:8080/api/productos/cat/')
    ...
    .then(data => {
      ...
      if (targetCombo) {
        ...
        defaultOption.value = ''; defaultOption.text = ''; defaultOption.selected = true; defaultOption.disabled = true; defaultOption.hidden = true;
  ...
  if (comboPages) {
    ...
    optionPages.value = ''; optionPages.text = ''; optionPages.selected = true; optionPages.disabled = true; optionPages.hidden = true;
  ...
  if (comboSort) {
    ...
    optionSort.value = ''; optionSort.text = ''; optionSort.selected = true; optionSort.disabled = true; optionSort.hidden = true;
